...
import { RIEPILOGO_MAX, hrefCalendarioCall, isCallInCorso, partiGiornoRelativo } from "@/lib/calendario/prossime";
  ...
  return (
    <section className="overflow-hidden rounded-brand-lg border border-brand-border bg-brand-elevated shadow-brand-md">
      <div className="flex items-center justify-between gap-3 border-b border-brand-border px-4 py-3 sm:px-5">
        ...
        <Link
          href={prossima ? hrefCalendarioCall(prossima.giorno) : "/dashboard/calendario"}
        ...
        <ul className="divide-y divide-brand-border">
          {visibili.map((call, index) => {
            ...
            return (
              <li key={call.id}>
                <Link
                  href={hrefCalendarioCall(call.giorno)}
      ...
      {nascoste > 0 && (
        <div className="border-t border-brand-border px-4 py-2.5 sm:px-5">
          <Link
            href={hrefCalendarioCall(calls[visibili.length]?.giorno ?? oggi)}
